...
function generateSteps(input: number[] | { adjacency: Record<string, string[]> }): VisualizationStep[] {
  ...
  steps.push({ type: 'array', array: arr, message: `Upper Bound: ${target} 초과인 첫 번째 위치를 찾습니다.` })
  ...
  if (lo < arr.length) {
    steps.push({ type: 'array', array: arr, sorted: [lo], pointers: [{ index: lo, label: '✓', color: '#10b981' }], message: `Upper Bound = ${lo} (arr[${lo}]=${arr[lo]})` })
  } else {
    steps.push({ type: 'array', array: arr, message: `Upper Bound = ${lo} (배열 끝)` })
  }
  return steps
}

export const upperBoundViz: VisualizationConfig = {
  name: 'Upper Bound',
  description: 'target 초과인 첫 번째 위치를 이분 탐색으로 찾습니다.',
